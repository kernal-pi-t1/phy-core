...
Usage:
    python select_crop_region.py              # Use live RealSense capture
    python select_crop_region.py --image PATH # Use existing image
...
Saves crop config to: crop_config.yaml
"""
...
CONFIG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "crop_config.yaml")
points = []
img_display = None
img_original = None
...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if len(points) < 2:
            ...
            # Draw rectangle if 2 points selected
            if len(points) == 2:
...
def load_crop_config(config_path=None):
    """Load crop config. Returns (x1, y1, x2, y2) or None if not found."""
